---
title: "Glider Deployment Report: glider (month day, year)"
format: 
  pdf: 
    toc: true
    colorlinks: true
...
deployment_year <- "yyyy"
deployment_date <- "Month dd, yyyy"
locale <- "" # "CCE", "Antarctica", "Hawaii"

battery_type <- "" # "lithium primary", "lithium primary extended", "lithium rechargeable", "lithium rechargeable extended"
battery_capacity <- "" # "720", "1050", "215", "300"
...
if (processor == "GCP") {
  ...
  raw.path <- file.path(home_dir, "gcs-mnt", "swfscesd-glider-deployments-data-in", deployment_year, deployment, "archive-sfmc")
  listma <- list.files(raw.path, pattern = "sample.*\\.ma$")
  listpacfg <- list.files(raw.path, pattern = "*_wispr.cfg")
  # listaacfg <- list.files(raw.path, pattern = "*_ad2cp.cfg")
  # listaacfg <- list.files(raw.path, pattern = "*_azfp.cfg")
  # listsgcfg <- list.files(raw.path, pattern = "*_solocam.cfg")
  ...
  raw.path <- here(base.path, deployment)
  listma   <- list.files(raw.path, pattern = "sample.*\\.ma$")
  # listpacfg <- list.files(raw.path, pattern="*_wispr.cfg")
  # listaacfg <- list.files(raw.path, pattern="*_ad2cp.cfg")
  # listaacfg <- list.files(raw.path, pattern="*_azfp.cfg")
  # listsgcfg <- list.files(raw.path, pattern="solocam.cfg")
}
...
if (length(listma) == num_sensors) {
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        if(length(display_name) == 0) display_name <- base_file # if missing file

        full_path <- file.path(raw.path, display_name[1])
    ...
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        if (length(matching_files) <= 1) {
          ...
          full_path <- file.path(raw.path, display_name)
          params <- parse_ma_file(full_path)
          ...
          for (m_file in matching_files) {
            full_path <- file.path(raw.path, m_file)
            params <- parse_ma_file(full_path)
    ...
    # sensor specific table ffor each sensor with more than one sample file
