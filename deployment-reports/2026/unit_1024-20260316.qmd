---
title: "Glider Deployment Report: unit_1024 (March 16, 2026)"
format: 
  pdf: 
    toc: true
    colorlinks: true
...
glider_serial <- "1024" 
deployment <-"unit_1024-20260316"
deployment_year <- "2026"
deployment_date <- "March 16, 2026"
locale <- "CCE" # "CCE", "Antarctica", "Hawaii"

battery_type <- "" # "lithium primary", "lithium primary extended", "lithium rechargeable", "lithium rechargeable extended"
...
if (length(listma) == num_sensors) {
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        full_path <- here::here(raw.path, deployment_year, deployment, "archive-sfmc", display_name[1])
    ...
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        if (length(matching_files) <= 1) {
          ...
          full_path <- here::here(base.path, deployment, display_name)
          params <- parse_ma_file(full_path)
